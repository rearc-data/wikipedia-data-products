...
while [[ $# -gt 0 ]]; do
  ...
  case ${opt} in
    ...
    "-s"|"--s3-prefix") export S3_PREFIX="$1"; shift;;
...
(cd code/pre-processing/pre-processing-code && zip -r pre-processing-code.zip . -x "*.dist-info/*" -x "bin/*" -x "**/__pycache__/*" -x "**/.ipynb_checkpoints/*")

#upload pre-preprocessing.zip to s3
echo "uploading pre-preprocessing.zip to s3"
aws s3 cp code/pre-processing/pre-processing-code/pre-processing-code.zip s3://$S3_BUCKET/$S3_PREFIX/$DATASET_NAME/automation/pre-processing-code.zip --region "$REGION" $PROFILE
...
echo "
{\"PRODUCT_CODE\":\"${slug}\",\"CATEGORY\":\"${l}\",\"S3_PREFIX\":\"${S3_PREFIX}\",\"SOURCE_URL\":\"${SOURCE_URL}\",\"DATASET_NAME\":\"${DATASET_NAME}\",\"DATASET_ARN\":\"${DATASET_ARN}\",\"DATASET_ID\":\"${DATASET_ID}\", \"PRODUCT_NAME\":\"${PRODUCT_NAME}\",\"PRODUCT_ID\":\"${PRODUCT_ID}\",\"SCHEDULE_CRON\":\"${SCHEDULE_CRON}\"}" >> "$products_info_file"
...
aws cloudformation create-stack --stack-name "$CFN_STACK_NAME" --template-body file://code/pre-processing/pre-processing-cfn.yaml --parameters ParameterKey=S3Bucket,ParameterValue="$S3_BUCKET" ParameterKey=S3Prefix,ParameterValue="$S3_PREFIX" ParameterKey=DataSetName,ParameterValue="$DATASET_NAME" ParameterKey=DataSetArn,ParameterValue="$DATASET_ARN" ParameterKey=ProductId,ParameterValue="$PRODUCT_ID" ParameterKey=Region,ParameterValue="$REGION" ParameterKey=SourceURL,ParameterValue="$SOURCE_URL" ParameterKey=ScheduleCron,ParameterValue="'$SCHEDULE_CRON'" --region "$REGION" --capabilities "CAPABILITY_AUTO_EXPAND" "CAPABILITY_NAMED_IAM" "CAPABILITY_IAM" $PROFILE
...
update () {
  ...
  aws cloudformation create-stack --stack-name "$CFN_STACK_NAME" --use-previous-template --parameters ParameterKey=S3Bucket,ParameterValue="$S3_BUCKET" ParameterKey=S3Prefix,ParameterValue="$S3_PREFIX" ParameterKey=DataSetName,ParameterValue="$DATASET_NAME" ParameterKey=DataSetArn,ParameterValue="$DATASET_ARN" ParameterKey=ProductId,ParameterValue="$NEW_PRODUCT_ID" ParameterKey=Region,ParameterValue="$REGION" ParameterKey=SourceURL,ParameterValue="$SOURCE_URL" ParameterKey=ScheduleCron,ParameterValue="$SCHEDULE_CRON" --region "$REGION" --capabilities "CAPABILITY_AUTO_EXPAND" "CAPABILITY_NAMED_IAM" "CAPABILITY_IAM" $PROFILE
